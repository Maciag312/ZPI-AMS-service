...
public class RoleTuple {
    ...
    public static RoleTuple fromDomain(Role role) {
        return new RoleTuple(role.getName(), role.getColor(), role.getPermissions().stream().map(PermissionTuple::fromDomain).collect(Collectors.toSet()));
    }
    ...
    public static Role toDomain(RoleTuple roleTuple) {
        ...
        return new Role(roleTuple.getName(), roleTuple.getColor(), permissions);
    }
}
